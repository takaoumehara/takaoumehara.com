...
1. **どの順で組み直すか。** 26ページを一度には出せない。候補の切り方:
   - (a) 採用担当が最初に見る順（work.html の並び上位から）
   - (b) 素材が揃っている順（画像30枚超: `value-frontier` `festival-design` `konosaki` `ela-quests` `odell-education` `kitadoko`）
   - (c) 直近の仕事から遡る順（2024–2026 → 2013）
2. **`marubatsu` は Variant C のままか、A に上げるか。**
3. **`verizon-ai-agents` の圧縮を先にやるか、後回しにするか。**（単独で1回分の作業量）
...
`assets/ai-window-deck/` は空。`docs/portfolio-case-study-ai-window-deck/case-study.md` §8 に
受け入れパスを書いてあるので、そのファイル名でコミットすれば表示に切り替わる。
...
| 1 | Q0 の回答で順番を確定 | 待ち |
| 2 | バッチ1（4〜5ページ）を v2 で再構築 | 待ち |
| 3 | `verizon-ai-agents` を10〜13枚に圧縮 | 待ち |
| 4 | preview 2枚を本ページに昇格し、preview を削除 | 待ち |
