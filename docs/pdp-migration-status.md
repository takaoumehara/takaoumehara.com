...
| ページ | 状態 |
|---|---|
| `verizon-ai-agents.html` | 部分的に v2。44枚・186KB。**圧縮は見送り決定**（§6）。枚数上限の例外扱い |
...
3. **`verizon-ai-agents` の圧縮を先にやるか、後回しにするか。** → **やらない**（2026-08-07 決定）。
   13枚版を作って見比べた結果、このページは44枚のまま据え置く。詳細は §6。
...
| 2 | `verizon-ai-agents` の圧縮 | **見送り** — §6。本番は44枚のまま |
...
## 6. `verizon-ai-agents` 圧縮版 — **見送り（2026-08-07）**

比較用に13枚版（`projects/verizon-ai-agents-compact.html`）を作ったが、
**「Verizon は縮小しなくてよい」と判断されたため削除した。** 本番ページ
`projects/verizon-ai-agents.html` は44枚のまま、**一度も変更していない**。

この案件は他と違って「量そのものが説得力」になっている、という判断。
今後 `case-study-format-audit.md` §2-2 の「10〜13枚」を適用する際も、
**このページは例外**として扱う。

圧縮版で分かったこと（将来また縮める話が出たとき用に記録）:

- 44枚 → 13枚は、既存の本文ブロックを選び直すだけで成立した（新規の文章は不要）。
- 各エージェント章の全画面タイトルカード4枚は、スライド内の小見出しに落とせる。
- 削れるのは各エージェントの深掘り31枚。骨格（診断 → 手法 → 4システム → 普及 → 成果 → 教訓）は13枚に収まる。

### 未解決（本番ページに残っている問題）

- WDS 5体のポートレート画像5枚が**リポジトリに存在しない**
  （`agent-mimir/saga/freya/idunn/eira-portrait.jpg`）。`onerror` で消える実装なので
  表示は壊れないが、色面だけが出ている状態。**圧縮版とは無関係の既存の問題。**
- Q4 の「42人 / 6ブランド / 10エージェント」が実測か見込みかは未回答。
